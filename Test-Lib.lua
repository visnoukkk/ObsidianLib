...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(16, 18, 28),
        MainColor = Color3.fromRGB(28, 30, 44),
        AccentColor = Color3.fromRGB(125, 85, 255),
        OutlineColor = Color3.fromRGB(120, 132, 168),
...
--// Creator Functions \\--
local GlassSchemeColors = {
    BackgroundColor = true,
    MainColor = true,
}

local function ApplyLiquidGlass(Object: GuiObject, Transparency: number?)
    ...
    Object.BackgroundTransparency = Transparency or Library.GlassTransparency or 0.5
end

local function AddGlassSheen(Parent: GuiObject)
    if not Parent then
        return
    end
    ...
    return Sheen, Gradient
end

function Library:SetWorldGlass(Enabled: boolean)
    local Info = Library.WindowAnimationInfo or TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    Enabled = Enabled == true

    if Library.WorldBlur then
        Library.WorldBlur.Enabled = true
        TweenService:Create(Library.WorldBlur, Info, {
            Size = Enabled and (Library.GlassBlurSize or 24) or 0,
        }):Play()

        if not Enabled then
            task.delay(Info.Time, function()
                if Library.WorldBlur and not Library.Toggled then
                    Library.WorldBlur.Enabled = false
                end
            end)
        end
    end
    ...
    if Library.WorldDim then
        ...
        if not Enabled then
            task.delay(Info.Time, function()
                if Library.WorldDim and not Library.Toggled then
                    Library.WorldDim.Enabled = false
                end
            end)
        end
    end
...
local function New(ClassName: string, Properties: { [string]: any }): any
    ...
    if Properties and Properties.BackgroundTransparency == nil then
        local BackgroundKey = Properties.BackgroundColor3
        if typeof(BackgroundKey) == "function" then
            ApplyLiquidGlass(Instance)
        elseif typeof(BackgroundKey) == "string" and GlassSchemeColors[BackgroundKey] then
            ApplyLiquidGlass(Instance)
        end
    end
...
do
    pcall(function()
        local ExistingBlur = Lighting:FindFirstChild("ObsidianLiquidGlassBlur")
        if ExistingBlur then
            ExistingBlur:Destroy()
        end

        local ExistingDim = Lighting:FindFirstChild("ObsidianLiquidGlassDim")
        if ExistingDim then
            ExistingDim:Destroy()
        end
    end)
    ...
    WorldBlur.Name = "ObsidianLiquidGlassBlur"
    WorldBlur.Enabled = false
    WorldBlur.Size = 0
    pcall(function()
        WorldBlur.Parent = Lighting
    end)
    Library.WorldBlur = WorldBlur

    local WorldDim = Instance.new("ColorCorrectionEffect")
    WorldDim.Name = "ObsidianLiquidGlassDim"
    WorldDim.Enabled = false
    WorldDim.Brightness = 0
    WorldDim.Contrast = 0
    WorldDim.Saturation = 0
    pcall(function()
        WorldDim.Parent = Lighting
    end)
    Library.WorldDim = WorldDim
end
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:Toggle(Value: boolean?)
        ...
        Library:SetWorldGlass(Library.Toggled)
...
function Library:Unload()
    ...
    if Library.WorldDim then
        pcall(function()
            Library.WorldDim:Destroy()
        end)
        Library.WorldDim = nil
    end
